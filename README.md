...
intelligence, and quantum computing — share a common algebraic foundation
in prime field arithmetic. Trident sits at their intersection because its
native `Field` type simultaneously satisfies the requirements of all three.
...
**Zero-knowledge cryptography** requires reversible computation with
complete arithmetic — every nonzero element must have a multiplicative
inverse, no information destroyed. This is the definition of a prime field.

**Quantum computing** requires unitary operations on state spaces with no
decoherence channels — no invariant subspaces that trap information. When
the dimension is prime, the state space has no nontrivial subgroups. Every
gate touches the full space. This is a prime-dimensional Hilbert space.

**Artificial intelligence** requires nonlinear functions over fixed-width
arithmetic for provable inference. Neural networks expressed in field
arithmetic produce STARK proofs alongside their outputs. Weights,
activations, and gradients are field elements from the start — no
float-to-field quantization, no precision loss.
...
it defines the native arithmetic of provable inference. Same structure.
Three readings.
...
[quantum computation](docs/explanation/quantum.md) and
[neural network inference](docs/explanation/vision.md). One design choice,
three futures.
...
**Three Pillars** — `std.nn` (field-native neural networks: matrix multiply,
attention, convolutions, lookup-table activations), `std.private` (ZK + FHE
+ MPC: credentials, auctions, voting, compliance-compatible privacy),
`std.quantum` (state management, gates, Grover's, QFT, VQE, error
correction).
