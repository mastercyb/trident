//! Float32 GPU-accelerated neural forward pass for evolutionary training.
//!
//! Converts Fixed (Goldilocks) weights to f32 on CPU, runs the entire forward
//! pass in native float32 on GPU. No field arithmetic emulation.
//! Single dispatch for all individuals × all blocks per chunk.
//!
//! The f32 path produces approximate fitness scores — sufficient for
//! evolutionary ranking. Exact field arithmetic is used only for saving
//! the best weights after each generation.
...
const WEIGHT_COUNT: u32 = 62080;

/// GPU params for f32 shader.
#[repr(C)]
...
impl F32Accelerator {
    /// Create f32 accelerator with capacity for `max_blocks` × `max_individuals`.
    pub fn try_create(max_blocks: u32, max_individuals: u32) -> Option<Self> {
        ...
        let shader_module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("neural_f32"),
            source: wgpu::ShaderSource::Wgsl(shaders::NEURAL_F32.into()),
        });

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("neural_f32_pipeline"),
            layout: None,
            module: &shader_module,
            entry_point: Some("neural_f32"),
            compilation_options: Default::default(),
            cache: None,
        });

        // No scratch buffer needed — f32 shader uses private (register) memory.
        // Limiting factor is weight buffer: max_individuals * 62080 * 4 bytes.
        let limits = device.limits();
        let max_buf = limits
            .max_buffer_size
            .min(limits.max_storage_buffer_binding_size as u64);

        // Weight buffer is the largest: individuals * WEIGHT_COUNT * 4
        let weight_bytes_per_ind = WEIGHT_COUNT as u64 * 4;
        let blocks_per_chunk = max_blocks; // no scratch → blocks not the bottleneck
                                           // But output buffer: individuals * blocks * MAX_OUTPUT * 4
        ...
        let blocks_per_chunk = blocks_per_chunk.min(blocks_by_output);

        // Block data: blocks_per_chunk * MAX_NODES * WORDS_PER_NODE * 4 (f32)
        ...
        let params_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("f32_params"),
            contents: bytemuck::bytes_of(&params),
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });

        let weight_buf_size = max_individuals as u64 * weight_bytes_per_ind;
        let weight_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("f32_weights"),
    ...
    /// Upload blocks for the next file. Converts node values to f32.
    pub fn upload_blocks(&mut self, blocks: &[TIRBlock]) {
        ...
        // Convert block nodes from u64 (Goldilocks fixed-point) to f32
        ...
        for block in blocks {
            for i in 0..slots_per_block {
                ...
                // Node values are raw u64 — convert via Fixed::to_f64
    ...
    /// Run forward pass for all individuals. Weights are Fixed, converted to f32 on CPU.
    /// Returns `[num_individuals][num_blocks][MAX_OUTPUT]` output codes.
    pub fn batch_forward(&self, weight_vecs: &[Vec<Fixed>]) -> Vec<Vec<Vec<u32>>> {
        ...
        // Convert all weights from Fixed to f32 — one bulk conversion
